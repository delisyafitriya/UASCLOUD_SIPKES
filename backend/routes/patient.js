...
router.post('/documents', upload.single('health_file'), async (req, res) => {
    ...
    try {
        ...
        const storage_url = cloudinaryResult.secure_url; // ✅ pakai secure_url (https)
        await pool.execute(
            'INSERT INTO medical_documents (patient_id, document_name, storage_url) VALUES (?, ?, ?)',
            [req.user.id, req.file.originalname, storage_url]
        );
        await writeAuditLog(req.user.id, 'UPLOAD_DOCUMENT', `Uploaded ${req.file.originalname} to Cloudinary`);
        res.json({ success: true, message: 'File uploaded successfully', document_url: storage_url });
    } catch (error) {
        console.error(error);
